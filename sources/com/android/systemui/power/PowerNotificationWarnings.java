...
public class PowerNotificationWarnings implements PowerUI.WarningsUI {
    ...
    private void updateNotification() {
        ...
        if (this.mInvalidCharger) {
            ...
            if (!EmergencyModeStatus.isEmergencyModeOn(this.mContext)) {
    ...
    private void showInvalidChargerNotification() {
        Notification.Builder color = new Notification.Builder(this.mContext, NotificationChannels.ALERTS).setSmallIcon(C0006R$drawable.ic_power_low).setWhen(0).setShowWhen(false).setOngoing(true).setContentTitle(this.mContext.getString(C0014R$string.invalid_charger_title)).setContentText(this.mContext.getString(C0014R$string.invalid_charger_text)).setColor(this.mContext.getColor(17170460));
    ...
    public void showWarningNotification() {
        ...
        String string = this.mContext.getString(C0014R$string.battery_low_title);
        if (this.mCurrentBatterySnapshot.isHybrid()) {
            str = getHybridContentString(format);
        } else {
            str = this.mContext.getString(C0014R$string.battery_low_percent_format, format);
        }
        Notification.Builder visibility = new Notification.Builder(this.mContext, NotificationChannels.BATTERY).setSmallIcon(C0006R$drawable.ic_power_low).setWhen(this.mWarningTriggerTimeMs).setShowWhen(false).setContentText(str).setContentTitle(string).setOnlyAlertOnce(true).setDeleteIntent(pendingBroadcast("PNW.dismissedWarning")).setStyle(new Notification.BigTextStyle().bigText(str)).setVisibility(1);
        ...
        if (!this.mPowerMan.isPowerSaveMode()) {
            visibility.addAction(0, this.mContext.getString(C0014R$string.battery_saver_start_action), pendingBroadcast("PNW.startSaver"));
        }
    ...
    private void showAutoSaverSuggestionNotification() {
        Notification.Builder contentText = new Notification.Builder(this.mContext, NotificationChannels.HINTS).setSmallIcon(C0006R$drawable.ic_power_saver).setWhen(0).setShowWhen(false).setContentTitle(this.mContext.getString(C0014R$string.auto_saver_title)).setContentText(this.mContext.getString(C0014R$string.auto_saver_text));
        ...
        contentText.addAction(0, this.mContext.getString(C0014R$string.no_auto_saver_action), pendingBroadcast("PNW.autoSaverNoThanks"));
    ...
    public void showHighTemperatureWarning() {
        if (!this.mHighTempWarning) {
            ...
            Notification.Builder color = new Notification.Builder(this.mContext, NotificationChannels.ALERTS).setSmallIcon(C0006R$drawable.ic_device_thermostat_24).setWhen(0).setShowWhen(false).setContentTitle(this.mContext.getString(C0014R$string.high_temp_title)).setContentText(this.mContext.getString(C0014R$string.high_temp_notif_message)).setVisibility(1).setContentIntent(pendingBroadcast("PNW.clickedTempWarning")).setDeleteIntent(pendingBroadcast("PNW.dismissedTempWarning")).setColor(Utils.getColorAttrDefaultColor(this.mContext, 16844099));
            SystemUI.overrideNotificationAppName(this.mContext, color, false);
    ...
    private void showHighTemperatureDialog() {
        if (this.mHighTempDialog == null) {
            ...
            systemUIDialog.setTitle(C0014R$string.high_temp_title);
            systemUIDialog.setMessage(C0014R$string.high_temp_dialog_message);
    ...
    private void showThermalShutdownDialog() {
        if (this.mThermalShutdownDialog == null) {
            SystemUIDialog systemUIDialog = new SystemUIDialog(this.mContext);
            systemUIDialog.setIconAttribute(16843605);
            systemUIDialog.setTitle(C0014R$string.thermal_shutdown_title);
            systemUIDialog.setMessage(C0014R$string.thermal_shutdown_dialog_message);
            systemUIDialog.setPositiveButton(17039370, null);
            systemUIDialog.setShowForAllUsers(true);
            systemUIDialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
    ...
    public void showThermalShutdownWarning() {
        Notification.Builder color = new Notification.Builder(this.mContext, NotificationChannels.ALERTS).setSmallIcon(C0006R$drawable.ic_device_thermostat_24).setWhen(0).setShowWhen(false).setContentTitle(this.mContext.getString(C0014R$string.thermal_shutdown_title)).setContentText(this.mContext.getString(C0014R$string.thermal_shutdown_message)).setVisibility(1).setContentIntent(pendingBroadcast("PNW.clickedThermalShutdownWarning")).setDeleteIntent(pendingBroadcast("PNW.dismissedThermalShutdownWarning")).setColor(Utils.getColorAttrDefaultColor(this.mContext, 16844099));
        SystemUI.overrideNotificationAppName(this.mContext, color, false);
    ...
    public void lambda$showUsbHighTemperatureAlarm$2$PowerNotificationWarnings() {
        if (this.mUsbHighTempDialog == null) {
            SystemUIDialog systemUIDialog = new SystemUIDialog(this.mContext, C0015R$style.Theme_SystemUI_Dialog_Alert);
            systemUIDialog.setCancelable(false);
            systemUIDialog.setIconAttribute(16843605);
            systemUIDialog.setTitle(C0014R$string.high_temp_alarm_title);
            systemUIDialog.setShowForAllUsers(true);
            systemUIDialog.setMessage(this.mContext.getString(C0014R$string.high_temp_alarm_notify_message, ""));
            ...
            systemUIDialog.setNegativeButton(C0014R$string.high_temp_alarm_help_care_steps, new DialogInterface.OnClickListener() {
    ...
    public /* synthetic */ void lambda$showUsbHighTemperatureAlarmInternal$5$PowerNotificationWarnings(DialogInterface dialogInterface, int i) {
        String string = this.mContext.getString(C0014R$string.high_temp_alarm_help_url);
    ...
    private void showStartSaverConfirmation(Bundle bundle) {
        if (this.mSaverConfirmation == null) {
            ...
            if (z) {
                systemUIDialog.setTitle(C0014R$string.battery_saver_confirmation_title_generic);
                systemUIDialog.setPositiveButton(17039828, new DialogInterface.OnClickListener(i, i2) {
                ...
                systemUIDialog.setTitle(C0014R$string.battery_saver_confirmation_title);
                systemUIDialog.setPositiveButton(C0014R$string.battery_saver_confirmation_ok, new DialogInterface.OnClickListener() {
    ...
    private CharSequence getBatterySaverDescription() {
        String charSequence = this.mContext.getText(C0014R$string.help_uri_battery_saver_learn_more_link_target).toString();
        if (TextUtils.isEmpty(charSequence)) {
            return this.mContext.getText(17039623);
        }
        SpannableString spannableString = new SpannableString(this.mContext.getText(17039624));
