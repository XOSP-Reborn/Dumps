...
public abstract class HeadsUpManager extends AlertingNotificationManager {
    ...
    public HeadsUpManager(final Context context) {
        ...
        this.mMinimumDisplayTime = resources.getInteger(C0008R$integer.heads_up_notification_minimum_time);
        this.mAutoDismissNotificationDecay = resources.getInteger(C0008R$integer.heads_up_notification_decay);
        this.mTouchAcceptanceDelay = resources.getInteger(C0008R$integer.touch_acceptance_delay);
        this.mSnoozedPackages = new ArrayMap<>();
        this.mSnoozeLengthMs = Settings.Global.getInt(context.getContentResolver(), "heads_up_snooze_length_ms", resources.getInteger(C0008R$integer.heads_up_default_snooze_length_ms));
    ...
    public void snooze() {
        for (String str : this.mAlertEntries.keySet()) {
            this.mSnoozedPackages.put(snoozeKey(getHeadsUpEntry(str).mEntry.notification.getPackageName(), this.mUser), Long.valueOf(this.mClock.currentTimeMillis() + ((long) this.mSnoozeLengthMs)));
        }
    }
    ...
    public class HeadsUpEntry extends AlertingNotificationManager.AlertEntry {
        ...
        public long calculateFinishTime() {
            return this.mPostTime + ((long) getRecommendedTimeoutMillis());
        }

        private int getRecommendedTimeoutMillis() {
            return HeadsUpManager.this.mAccessibilityMgr.getRecommendedTimeoutMillis(((AlertingNotificationManager) HeadsUpManager.this).mAutoDismissNotificationDecay, 7);
        }
    }
}
