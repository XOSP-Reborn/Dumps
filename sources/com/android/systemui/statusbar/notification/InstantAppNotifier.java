...
public class InstantAppNotifier extends SystemUI implements CommandQueue.Callbacks, KeyguardMonitor.Callback {
    ...
    private final Consumer<Boolean> mDockedListener = new Consumer() {
        /* class com.android.systemui.statusbar.notification.$$Lambda$InstantAppNotifier$xtVHKTfUxObWlx6TMvVdHY7npdU */
    ...
    private final TaskStackChangeListener mTaskListener = new TaskStackChangeListener() {
        /* class com.android.systemui.statusbar.notification.InstantAppNotifier.AnonymousClass2 */
    ...
    private void postInstantAppNotif(String str, int i, ApplicationInfo applicationInfo, NotificationManager notificationManager, int i2) {
        ...
        bundle.putString("android.substName", this.mContext.getString(C0014R$string.instant_apps));
        this.mCurrentNotifs.add(new Pair<>(str, Integer.valueOf(i)));
        String string = this.mContext.getString(C0014R$string.instant_apps_help_url);
        boolean z = !string.isEmpty();
        Context context = this.mContext;
        if (z) {
            i3 = C0014R$string.instant_apps_message_with_help;
        } else {
            i3 = C0014R$string.instant_apps_message;
        }
        String string2 = context.getString(i3);
        UserHandle of = UserHandle.of(i);
        Notification.Action build = new Notification.Action.Builder((Icon) null, this.mContext.getString(C0014R$string.app_info), PendingIntent.getActivityAsUser(this.mContext, 0, new Intent("android.settings.APPLICATION_DETAILS_SETTINGS").setData(Uri.fromParts("package", str, null)), 0, null, of)).build();
        if (z) {
            str2 = "android.intent.action.VIEW";
            action = build;
            pendingIntent = PendingIntent.getActivityAsUser(this.mContext, 0, new Intent("android.intent.action.VIEW").setData(Uri.parse(string)), 0, null, of);
            i4 = i2;
        } else {
            str2 = "android.intent.action.VIEW";
            action = build;
        ...
        if (taskIntent == null || !taskIntent.isWebIntent()) {
            ...
            builder = builder2;
            builder.addAction(new Notification.Action.Builder((Icon) null, this.mContext.getString(C0014R$string.go_to_web), PendingIntent.getActivityAsUser(this.mContext, 0, addCategory.addCategory("unique:" + System.currentTimeMillis()).putExtra("android.intent.extra.PACKAGE_NAME", applicationInfo.packageName).putExtra("android.intent.extra.VERSION_CODE", applicationInfo.versionCode & Integer.MAX_VALUE).putExtra("android.intent.extra.LONG_VERSION_CODE", applicationInfo.longVersionCode).putExtra("android.intent.extra.INSTANT_APP_FAILURE", activityAsUser), 0, null, of)).build());
        }
