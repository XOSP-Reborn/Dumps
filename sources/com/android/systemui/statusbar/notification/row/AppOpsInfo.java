...
public class AppOpsInfo extends LinearLayout implements NotificationGuts.GutsContent {
    ...
    private void bindHeader() {
        ...
        ((ImageView) findViewById(C0007R$id.pkgicon)).setImageDrawable(drawable);
        ((TextView) findViewById(C0007R$id.pkgname)).setText(this.mAppName);
    }

    private void bindPrompt() {
        ((TextView) findViewById(C0007R$id.prompt)).setText(getPrompt());
    }

    private void bindButtons() {
        findViewById(C0007R$id.settings).setOnClickListener(new View.OnClickListener() {
        ...
        ((TextView) findViewById(C0007R$id.ok)).setOnClickListener(this.mOnOk);
    }
    ...
    private String getPrompt() {
        ...
        if (this.mAppOps.size() == 1) {
            if (this.mAppOps.contains(26)) {
                return ((LinearLayout) this).mContext.getString(C0014R$string.appops_camera);
            }
            if (this.mAppOps.contains(27)) {
                return ((LinearLayout) this).mContext.getString(C0014R$string.appops_microphone);
            }
            return ((LinearLayout) this).mContext.getString(C0014R$string.appops_overlay);
        } else if (this.mAppOps.size() != 2) {
            return ((LinearLayout) this).mContext.getString(C0014R$string.appops_camera_mic_overlay);
        } else {
            if (!this.mAppOps.contains(26)) {
                return ((LinearLayout) this).mContext.getString(C0014R$string.appops_mic_overlay);
            }
            if (this.mAppOps.contains(27)) {
                return ((LinearLayout) this).mContext.getString(C0014R$string.appops_camera_mic);
            }
            return ((LinearLayout) this).mContext.getString(C0014R$string.appops_camera_overlay);
        }
    }
