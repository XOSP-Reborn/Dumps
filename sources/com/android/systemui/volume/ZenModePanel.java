...
public class ZenModePanel extends FrameLayout {
    ...
    public ZenModePanel(Context context, AttributeSet attributeSet) {
        ...
        this.mZenModeConditionLayoutId = C0010R$layout.zen_mode_condition;
        this.mZenModeButtonLayoutId = C0010R$layout.zen_mode_button;
        if (DEBUG) {
    ...
    public void createZenButtons() {
        this.mZenButtons = (SegmentedButtons) findViewById(C0007R$id.zen_buttons);
        this.mZenButtons.addButton(C0014R$string.interruption_level_none_twoline, C0014R$string.interruption_level_none_with_warning, 2);
        this.mZenButtons.addButton(C0014R$string.interruption_level_alarms_twoline, C0014R$string.interruption_level_alarms, 3);
        this.mZenButtons.addButton(C0014R$string.interruption_level_priority_twoline, C0014R$string.interruption_level_priority, 1);
    ...
    public void onFinishInflate() {
        ...
        createZenButtons();
        this.mZenIntroduction = findViewById(C0007R$id.zen_introduction);
        this.mZenIntroductionMessage = (TextView) findViewById(C0007R$id.zen_introduction_message);
        this.mZenIntroductionConfirm = findViewById(C0007R$id.zen_introduction_confirm);
        ...
        this.mZenIntroductionCustomize = (TextView) findViewById(C0007R$id.zen_introduction_customize);
        ...
        this.mConfigurableTexts.add(this.mZenIntroductionCustomize, C0014R$string.zen_priority_customize_button);
        this.mZenConditions = (LinearLayout) findViewById(C0007R$id.zen_conditions);
        this.mZenAlarmWarning = (TextView) findViewById(C0007R$id.zen_alarm_warning);
        this.mZenRadioGroup = (RadioGroup) findViewById(C0007R$id.zen_radio_buttons);
        this.mZenRadioGroupContent = (LinearLayout) findViewById(C0007R$id.zen_radio_buttons_content);
        this.mEdit = (ViewGroup) findViewById(C0007R$id.edit_container);
        ...
        this.mAutoRule = findViewById(C0007R$id.auto_rule);
    ...
    private void updateWidgets() {
        ...
        if (z) {
            if (z2) {
                i = C0014R$string.zen_priority_introduction;
            } else if (z4) {
                i = C0014R$string.zen_alarms_introduction;
            } else if (this.mVoiceCapable) {
                i = C0014R$string.zen_silence_introduction_voice;
            } else {
                i = C0014R$string.zen_silence_introduction;
            }
    ...
    private String computeAlarmWarningText(boolean z) {
        ...
        if (condition == null || isForever(condition)) {
            i = C0014R$string.zen_alarm_warning_indef;
        } else {
            long tryParseCountdownConditionId = ZenModeConfig.tryParseCountdownConditionId(this.mSessionExitCondition.id);
            i = (tryParseCountdownConditionId <= currentTimeMillis || nextAlarm >= tryParseCountdownConditionId) ? 0 : C0014R$string.zen_alarm_warning;
        }
        if (i == 0) {
            return null;
        }
        ...
        return getResources().getString(i, getResources().getString(z2 ? C0014R$string.alarm_template : C0014R$string.alarm_template_far, DateFormat.format(DateFormat.getBestDateTimePattern(Locale.getDefault(), z2 ? is24HourFormat ? "Hm" : "hma" : is24HourFormat ? "EEEHm" : "EEEhma"), nextAlarm)));
    }
    ...
    private static String foreverSummary(Context context) {
        return context.getString(17041744);
    }
    ...
    private void announceConditionSelection(ConditionTag conditionTag) {
        ...
        int selectedZen = getSelectedZen(0);
        if (selectedZen == 1) {
            str = this.mContext.getString(C0014R$string.interruption_level_priority);
        } else if (selectedZen == 2) {
            str = this.mContext.getString(C0014R$string.interruption_level_none);
        } else if (selectedZen == 3) {
            str = this.mContext.getString(C0014R$string.interruption_level_alarms);
        } else {
            return;
        }
        announceForAccessibility(this.mContext.getString(C0014R$string.zen_mode_and_condition, str, conditionTag.line1.getText()));
    }
    ...
    public final class ZenPrefs implements SharedPreferences.OnSharedPreferenceChangeListener {
        ...
        private ZenPrefs() {
            this.mNoneDangerousThreshold = ZenModePanel.this.mContext.getResources().getInteger(C0008R$integer.zen_mode_alarm_warning_threshold);
