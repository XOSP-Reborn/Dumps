...
public class MaterialShapeDrawable extends Drawable implements TintAwareDrawable, ShapeAppearanceModel.OnChangedListener {
    ...
    private PorterDuffColorFilter calculatePaintColorTintFilter(Paint paint, boolean z) {
        int color;
        int layerElevationOverlayIfNeeded;
        if (!z || (layerElevationOverlayIfNeeded = layerElevationOverlayIfNeeded((color = paint.getColor()))) == color) {
            return null;
        }
        return new PorterDuffColorFilter(layerElevationOverlayIfNeeded, PorterDuff.Mode.SRC_IN);
    }
